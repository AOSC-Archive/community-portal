...
$langs = array(
  ...
  'zh-cn' => array(
    ...
    'way-tarball-detail'=>'除了基于互联网的 LiveKit 的安装方式，你还可以直接使用 Tarball 发行进行安装，这种方式可以更快的获得最新版本的 AOSC OS。
                一般情况下，你需要进行下列操作：',
    'way-tarball-step-1'=>'寻找一个可启动的发行版 LiveCD，必须是 x86_64 架构（64位系统）。除了 Ubuntu、Debian 等较大的发行版外，ArchLinux、Gentoo 的 Live 镜像也都是不错的选择；',
    ...
    'dpkg-gnome'=>'AOSC OS with GNOME。GNOME 是一个完全免费且开源的桌面环境。在GNOME基金会的帮助下，它配套拥有众多应用套件。
                    GNOME被许多主流发行版如 Fedora 选为默认的桌面环境。',
    'dl'=>'下载',
    'dl-link'=>'下载最新 Tarball 发行',
    'ss-link'=>'截屏',
    'dpkg-xfce'=>'AOSC OS with XFCE。XFCE 是一个轻量级, 并且优雅易用的桌面环境。XFCE基于GTK+ 2的工具链接口进行构建。GTK+ 2也是GTK+ 3的前身，后者正在被GNOME所使用。XFCE相对来说会在老机器上运行的更快，甚至于可以跑在2001年的机器上（对于AOSC OS来说，这个发行版只要求机器支持安装64位操作系统）。XFCE适配尖端技术，如systemd-logind',
